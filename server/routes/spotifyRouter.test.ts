...
describe("Testing spotifyRouter", () => {
    ...
    describe("GET /artist/:id", () => {
        ...
        it("gets stam1na", async () => {
            const response = await request.get(`${baseURL}/artist/${stam1naBand}`)
    ...
    describe("GET /artist/:id/albums", () => {
        ...
        it("gets artists albums", async () => {
            const response = await request.get(`${baseURL}/artist/${stam1naBand}/albums`)
            .set('Accept', 'application/json')
            .set("Content-Type", 'application/json')
            .set("Authorization", access_token)
            expect(response.status).toBe(200);
            expect(response.body.filter(s => s.name ==="Elokuutio")[0].id).toBe(elokuutioAlbum)
        })
    });
