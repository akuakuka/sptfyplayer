...
import {spotifyArtist,spotifyAlbum, spotifyTrack, spotifyItem} from "../../../server/types/SpotifyTypes"
...
const Item:React.FC<spotifyItem> = ({images,name,id,type}) => {
    ...
    const handlePlayAlbum = async () => {
        ...
        play(user,device?.device_id,getTrackUrisFromAlbum(detailedAlbum.tracks.items))
      }
    ...
    return (
        ...
        {type === "album" ? 
         <> {images[0] && <Image src={images ? images[0].url : "https://via.placeholder.com/150"} boxSize="150px" onClick={() => handlePlayAlbum()} cursor="pointer"/> }</>
          : 
          <> {images[0] &&  <Avatar src={images ? images[0].url : "https://via.placeholder.com/150"} boxSize="150px" boxShadow="dark-lg" />}</>
        }
        <Text>{name}</Text>
            </Flex>
